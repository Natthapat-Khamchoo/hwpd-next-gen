...
**ห้ามตั้ง `PASSWORD_PEPPER`** รหัสผ่านทุกบัญชีใน `tb_Users` ถูก hash ด้วยค่าเริ่มต้น
ในโค้ด ตั้งค่าอื่นทับเมื่อไหร่ ทุกคนล็อกอินไม่ได้ทันที

(ปัจจุบัน 101 บัญชี = ประจำหน่วย 52 + ผู้ปฏิบัติ 43 + บัญชีทดสอบ 6 เลขนี้เพิ่มได้เรื่อย ๆ
นับจริงจาก `tb_Users` อย่ายึดตัวเลขในเอกสาร)
