...
บัญชีระดับ บก. สถานี `"00"` ที่ทำได้เฉพาะงานประชาสัมพันธ์ หน้าแรกคือ `PrCenterDashboard`
ซึ่งมีตัวเลือก กก. อยู่ด้านบน แล้วส่ง `station` นั้นไปกับทุกคำขอ

**ข้อจำกัดบังคับที่ backend ไม่ใช่ที่การซ่อนเมนู** — `check_station_match("00", สถานีใดก็ได้)`
คืน `True` เสมอ ด่านขอบเขตสถานีที่ endpoint ฝั่งอ่านส่วนใหญ่ใช้จึงไม่กันบัญชีนี้เลย
ถ้าพึ่งแค่การไม่มีปุ่มให้กด คนที่เปิด DevTools เป็นจะอ่านรายงานจับกุม ยอดน้ำมัน
และกำลังพลของทั้งแปดกองได้

`_enforce_pr_only()` ใน `current_session` จึงเป็นด่านจริง เขียนเป็น **allowlist**
(`PR_ONLY_ALLOWED_PREFIXES` / `PR_ONLY_ALLOWED_EXACT`) เพื่อให้ endpoint ที่เพิ่มวันหลัง
ถูกปิดโดยปริยาย ไม่ใช่เปิดแล้วรอให้มีคนนึกได้ว่าต้องไปเพิ่มในรายการที่ห้าม

| เรียกได้ | ปิดหมด |
|---|---|
| `/api/pr/*` ทุกเส้น | ทุกเส้นที่เหลือ ตอบ 403 |
| `/api/login`, `/api/logout`, `/api/change-password` | รวมถึงรายงานปฏิบัติการ กำลังพล น้ำมัน ของกลาง แผนที่ สืบค้น ทะเบียนผู้ใช้ |

`tests/test_pr_only_role.py` กวาดทุก route ของแอปแล้วยืนยันว่าไม่มีเส้นไหนหลุด
และตรึงไว้ว่ามี endpoint เดียวที่ตรวจโทเคนเองโดยไม่ผ่าน `current_session`
(`/api/admin/aggregate-status` ซึ่งกันด้วย `NATIONAL_VIEW_ROLES` ของตัวเองอยู่แล้ว)

| หน้า | endpoint | อ่านจาก |
|---|---|---|
| `PrCenterDashboard` เลือก กก. | `GET /api/pr/divisions` | `DB_ROUTER` ไม่แตะชีต |
