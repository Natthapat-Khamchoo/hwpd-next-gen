...
def build_perfect_10_col_workbook(div_num: int, stations: list, output_dir: str):
    ...
    # PERFECT 10 COLUMNS (Phone Number Included!)
    # 1. ลำดับ
    # 2. ยศ - ชื่อ - นามสกุล
    # 3. เบอร์โทรศัพท์
    # 4. สถานีตำรวจทางหลวง (ส.ทล.)
    # 5. หน่วยบริการ / ตู้สายตรวจ
    # 6. ตำแหน่ง / สิทธิ์ในระบบ
    # 7. ต้นทางที่ส่งมาช่วยราชการ
    # 8. ปลายทางที่ไปช่วยราชการ
    # 9. วันที่เริ่มช่วยราชการ
    # 10. วันที่สิ้นสุดช่วยราชการ
    headers = [
        "ลำดับ",
        "ยศ - ชื่อ - นามสกุล",
        "เบอร์โทรศัพท์",
    ...
    for st in stations:
        ...
        ws["A2"] = "* กรุณากรอก ยศ-ชื่อ-สกุล เบอร์โทรศัพท์ หน่วยบริการ ตำแหน่ง (กรณีช่วยราชการให้ระบุหน่วยงานต้นทาง/ปลายทาง และวันเริ่ม-สิ้นสุด)"
    ...
    wb.save(file_path)
    print(f"Generated Fresh 10-Column Workbook: {file_path}")


if __name__ == "__main__":
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    
    # Generate in new clean directories to avoid any open file locks!
    out_dir_main = os.path.join(base_dir, "แบบฟอร์มฉบับใหม่_แยกตาม_กก")
    os.makedirs(out_dir_main, exist_ok=True)

    for div_num, stations in STATIONS_DATA.items():
        build_perfect_10_col_workbook(div_num, stations, out_dir_main)

    print(f"\nALL 10-COLUMN WORKBOOKS GENERATED SUCCESSFULLY IN: {out_dir_main}")
